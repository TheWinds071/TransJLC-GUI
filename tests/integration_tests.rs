...
const KICAD_TEST_FILES: &[(&str, &str)] = &[
    ("project-F_Cu.gbr", "G04 KiCad test*\nG01*\nD10*\nG04 End*\n"),
    ("project-B_Cu.gbr", "G04 KiCad test*\nG01*\nD11*\nG04 End*\n"),
    ("project-F_Mask.gbr", "G04 KiCad mask*\nG01*\nD12*\nG04 End*\n"),
    ("project-Edge_Cuts.gbr", "G04 KiCad outline*\nG01*\nD13*\nG04 End*\n"),
...
fn test_auto_detection_kicad() {
    ...
    let patterns = PatternMatcher::auto_detect_eda(&files)
        .expect("Should detect KiCad format");
...
fn test_auto_detection_protel() {
    ...
    let patterns = PatternMatcher::auto_detect_eda(&files)
        .expect("Should detect Protel format");
...
fn test_gerber_processor_kicad_conversion() {
    ...
    let test_content = "G04 Test file*\nD10*\nG01X100Y100D01*\nD11*\nG54D12*\n%ADD13C,0.1*%\nD14*\nM02*".to_string();
    
    let result = processor.process_gerber_content(test_content, true)
...
fn test_gerber_processor_header_addition() {
    ...
    let result = processor.process_gerber_content(test_content, false)
...
fn test_gerber_processor_large_file_handling() {
    ...
    let result = processor.process_gerber_content(large_content.clone(), false)
...
fn test_config_eda_type_parsing() {
    let test_cases = vec![
        ...
        ("jlc", EdaType::Jlc),
        ("custom_format", EdaType::Custom("custom_format".to_string())),
    ];
